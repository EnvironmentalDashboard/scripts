...
//   cron($db, $bos, $meter, $res, $amount, $user['id'], false, false, false); // Will update all the meters associated with the $user_id and (gauges_using > 0 OR for_orb > 0 OR orb_server > 0 OR timeseries_using > 0)
// }

$options = getopt('', array('user_id:'));
if (array_key_exists('user_id', $options)) {
  echo "hour.php: Updating meters with {$res}-res data for user #{$options['user_id']}\n";
  echo date('F j, Y, g:i a') . "\n";
  $bos = new BuildingOS($db, $options['user_id']);
  $meter = new Meter($db);
  $bos->cron($meter, $res, $amount);
} else {
  echo "hour.php: Please provide a user_id\n";
}
?>