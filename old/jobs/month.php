...
//   cron($db, $bos, $meter, $res, $amount, $user['id'], false, true, false); // Will update all the meters associated with the $user_id and id IN (SELECT meter_id FROM saved_chart_meters) OR id IN (SELECT meter_id FROM gauges) OR bos_uuid IN (SELECT elec_uuid FROM orbs) OR bos_uuid IN (SELECT water_uuid FROM orbs) OR bos_uuid IN (SELECT DISTINCT meter_uuid FROM relative_values WHERE permission = 'orb_server')
// }

$options = getopt('', array('user_id:'));
if (array_key_exists('user_id', $options)) {
  echo "month.php: Updating meters with {$res}-res data for user #{$options['user_id']}\n";
  echo date('F j, Y, g:i a') . "\n";
  $bos = new BuildingOS($db, $options['user_id']);
  $meter = new Meter($db);
  $bos->cron($meter, $res, $amount);
} else {
  echo "month.php: Please provide a user_id\n";
}
?>