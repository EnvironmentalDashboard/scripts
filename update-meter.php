...
if (php_sapi_name() === 'cli') { // script is being run from command line
  $options = getopt('', array('org_id:', 'meter_id:', 'meter_uuid:', 'meter_url:', 'res:'));
  foreach (array('org_id', 'meter_id', 'meter_uuid', 'meter_url', 'res') as $required_option) {
  ...
  if ($ok) {
    $bos = new BuildingOS($db, $options['org_id']);
    var_dump($bos->updateMeter($options['meter_id'], $options['meter_uuid'], $options['meter_url'] . '/data', $options['res'], new Meter($db), true));
  } else {
    echo 'Usage: php update-meter.php --org_id="1" --meter_id="800" --meter_uuid="oberlin_city_prospect_main_e" --meter_url="https://api.buildingos.com/meters/oberlin_city_prospect_main_e" --res="live"';
    echo "\n";
  }
